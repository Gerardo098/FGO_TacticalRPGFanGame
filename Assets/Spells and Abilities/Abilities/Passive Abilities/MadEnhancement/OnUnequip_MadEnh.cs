...
/// </summary>
[CreateAssetMenu(menuName = "Effect Triggers/Mad Enhancement/On Unequip")]
public class OnUnequip_MadEnh : Trigger_OnUnequip
{
    int bonus; // Bonus MP gen we should've had

    public override bool Activate(AbilityEffect effect, CharacterManager unit = null)
    {
        // Upon unequiping this passive ability, find the bonus
        RankRead(effect.GetCurrentRank());

        // Remove the bonus from the unit's MP Generation stat
        Parameter MPGen = unit.MPGen;
        unit.character.parameterContainer.Subtract(MPGen, bonus);
